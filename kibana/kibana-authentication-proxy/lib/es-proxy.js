...
function proxyRequest(request, response, host, port, user, password, winston, getProxiedRequestPath, isUI) {
  var filteredHeaders = {};
  ...
  proxyReq.addListener('error', function(err){
    ...
    if (winston) { winston.error('ElasticSearch Server Error: ' + err.code);}
  });
...
exports.configureESProxy = function(app, basePath, esHost, esPort, esUser, esPassword, winston) {
  app.use(path.join(basePath, "/__es"), function(request, response, next) {
    proxyRequest(request, response, esHost, esPort, esUser, esPassword, winston,
      function getProxiedRequestPath(request) {
        return request.url;
      });
  });
  app.use(path.join(basePath, "/_plugin"), function(request, response, next) {
    proxyRequest(request, response, esHost, esPort, esUser, esPassword, null,
      function getProxiedRequestPath(request) {
